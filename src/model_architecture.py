...
class FasterRCNNModel:
    def __init__(self,num_classes,device):
        ...
        logger.info('Model Architecture initalized ...')
    ...
    def complie(self,lr=1e-04):
        try:
            self.optimizer=Adam(self.model.parameters(),lr=lr)
            logger.info("model complied Successfully") 
        except Exception as e:
            logger.error(f'Failed to compile model {e}')
            raise CustomException('Failed to compile model',e) 
    def train(self,train_loder,num_epochs=10):
        try:
            self.model.train()
            for epoch in range(1,num_epochs):
                total_loss=0
                logger.info(f'Epoch {epoch} started ..')
                for images,targets in tqdm(train_loder,desc=f'Epoch {epoch}'):
